---
title: "Exam 2 of Intro to ML"
author: "SHUBHAM SINGH, SOUMIK CHOUDHURI, HARSH MEHTA, KARTHICK RAMASUBRAMANIAN"
date: "08/16/2021"
output:
...
specific flight started from and ended at Austin and we print the number of flights to and from Austin
...
We did some extra analysis of the flights data and plotted a few graphs.

```{r ans b-1-beta, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}

library(dplyr)

abia = read.csv('ABIA.csv')

#created a date column since this is a time series dataset
abia$Date = paste(abia$Year, abia$Month,abia$DayofMonth,sep='-')
abia$week = cut(abia$DayofMonth,c(0,7,14,21,28,31),include.lowest = TRUE,labels = 1:5)
```


```{r }
temp = abia %>% count(Date)
plot(temp$n, type='l',xlab='Days from 1st January',ylab='Number of flights')
```

#Observations
There seems to be a cyclical pattern in the daily number of flights departing and arriving in
Austin when we look at the entire year. During its busy days, Austin can have upwards of 400
flights departing. 

```{r ans b-1-beta-2, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia %>% count(Month)
plot(temp$Month, temp$n, type='b',xlab='Month',ylab='Number of Flights')
```

#Observations

Highest number of flights taken in May, June and July
Significant drop in flights after August - about the time when Lehman
brothers collapsed during the financial crisis. 

```{r ans b-1-beta-3, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia %>% count(Month,DayOfWeek)
boxplot(temp$n~temp$Month, xlab = 'Month', ylab = 'Number of Flights')

```

#Observations
Broader range for number of flights per week in January and July. Tight range in almost all months but especially in February, August and November. 
```{r ans b-1-beta-4, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
boxplot(temp$n~temp$DayOfWeek, xlab = 'Day of the Week', ylab = 'Number of Flights')

```

#Observations
Similar number of flights depart and arrive in Austin every week day with the exception of
flights on Saturday and Sunday.

```{r ans b-1-beta-5, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia %>% count(Month,week)
boxplot(temp$n~temp$week, xlab = 'Week in Month',ylab = 'Number of Flights')
```

#Observations
5 week of the month only has at most 3 days.
Similar number of flights depart and arrive in Austin every week (for all months) 


For flights originating from Austin

```{r ans b-1-beta-6, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
abia2 = filter(abia, Origin == 'AUS')
temp = abia2 %>%
  group_by(DayOfWeek) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
plot(temp$DayOfWeek,temp$mean_dep_delay, type="l",col="red",ylab='Delay in minutes',xlab = 'Day of Week',ylim=c(0,15) )
lines(temp$DayOfWeek,temp$mean_arr_delay,col="green", type='l')
legend(3,15,legend=c("Dep Delay", "Arr Delay "),
       col=c("red", "green"), lty=c(1,1), cex=0.8)
```

#Observations
Highest arrival and thus departure delay on Fridays, lowest on Mondays and Saturdays

```{r ans b-1-beta-16, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia2 %>%
  group_by(Month) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
plot(temp$Month,temp$mean_dep_delay, type="l",col="red",ylab='Delay in minutes',xlab = 'Month',ylim=c(-1,15) )
lines(temp$Month,temp$mean_arr_delay,col="green")
legend(8,15,legend=c("Dep Delay", "Arr Delay "),
       col=c("red", "green"), lty=c(1,1), cex=0.8)
```

#Observations
Significant drop in delay in the months of September to November. It is interesting because one 
would expect more delays in the winter months. The summer months probably see more traffic
(as confirmed by our plot above) and thus more delays. 

```{r ans b-1-beta-26, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia2 %>%
  group_by(week) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
temp$week = as.numeric(temp$week)
plot(temp$week,temp$mean_arr_delay,col="green",type="l",ylab='Delay in minutes',xlab = 'week',ylim=c(-1,15))
lines(temp$week,temp$mean_dep_delay, col='red')
legend(3,15,legend=c("Dep Delay", "Arr Delay "),
       col=c("green", "red"), lty=c(1,1), cex=0.8)
```

#Observations
Week 3 of the month has more delays compared to the rest of the weeks in the month


```{r ans b-1-beta-36, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia2 %>%
  group_by(Month,week) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
temp$week = as.factor(temp$week)
plot(temp$week,temp$mean_dep_delay, type="l",col="red",ylab='Dep Delay in minutes',xlab = 'week')
plot(temp$week,temp$mean_arr_delay, type="l",col="green",ylab='Arr Delay in minutes',xlab = 'week')
```

#Observations
Week 3 of the month has more delays compared to the rest of the weeks in the month

```{r ans b-1-beta-46, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp$Month = as.factor(temp$Month)
plot(temp$Month,temp$mean_dep_delay, type="l",col="red",ylab='Dep Delay in minutes',xlab = 'month')
plot(temp$Month,temp$mean_arr_delay, type="l",col="green",ylab='Arr Delay in minutes',xlab = 'month')
plot(temp$Month,(temp$mean_dep_delay - temp$mean_arr_delay), type="l",col="blue",ylab='Dep Delay - Arr Delay in minutes',xlab = 'month')
```
#Observations
Though Week 3 of the month has more mean departure delay time compared to the rest of the weeks in the month the median departure delay time is the comparable

```{r ans b-1-beta-56, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia %>%
  group_by(UniqueCarrier) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
temp$UniqueCarrier = as.factor(temp$UniqueCarrier)
plot(temp$mean_dep_delay, type="b",col="red",ylab='Dep Delay in minutes',xlab = 'Carrier',ylim=c(0,20),xaxt='n')
axis(side =1, seq(1:16), labels = unique(temp$UniqueCarrier))
lines(temp$UniqueCarrier, temp$mean_arr_delay, type="b",col="green")
```
#Observations
Some carriers have significantly more delays than the others


```{r ans b-1-beta-66, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
temp = abia2 %>%
  group_by(Dest) %>%
  summarize(mean_arr_delay = mean(ArrDelay, na.rm = TRUE), 
            mean_dep_delay = mean(DepDelay, na.rm = TRUE))
barplot(temp$mean_dep_delay, names.arg = temp$Dest, xlab='DEST', ylab = 'Departure Delay')
barplot((temp$mean_dep_delay-temp$mean_arr_delay), names.arg = temp$Dest, xlab='Dest',ylab='Dep - Arrival Delay')
```
#Observations

Arrival and Departure Delay to different destinations for flights originating from Austin


```{r ans b-1-beta-7, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
cor(abia$ArrDelay,abia$DepDelay,use="complete.obs")
```
Inference - A flight that has arrived late is very likely to depart late



```{r ans b-1-beta-8, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
plot(abia$ArrDelay, abia$DepDelay, xlab='Arrival Delay', ylab='Departure Delay')

```

- Linear relationship between arrival delay and departure delay


```{r ans b-1-beta-9, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
plot(abia$Distance, abia$TaxiOut, xlab = 'Distance',ylab='Taxi Out')
```
There does not seem to be a pattern between these two variables.




```{r ans b-13-alpha, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
library(OpenImageR)

file1 <- file.path(getwd(), 'Destination_Austin_plot.png')
image = readImage(file1)
imageShow(image)
```


```{r ans b-1-alpha, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
file2 <- file.path(getwd(), 'Origin_Austin_plot.png')
image2 = readImage(file2)
imageShow(image2)
...
```
- We plotted the locations on the US map for the flights arriving at Austin and for those departing from Austin


```{r ans b-1-alpha-22, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
origin <- abia %>%
  distinct(Origin) %>%
  rename(label = Origin)
...
nodes <- full_join(origin, destinations, by = "label")
...
ggraph(routes_tidy, layout = "graphopt") +
  ...
  labs(edge_width = "Flights") +
  theme_graph(foreground = 'steelblue', fg_text_colour = 'white', base_family = 'Helvetica')
```
#Observations
This graph illustrates the number of flights from Austin to several other destinations and 
from other destinations to Austin in 2008. Most flights either arrive from or go to
Dallas, Houston and Phoenix, Denver, Chicago

```{r ans b-1-alpha-23, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
...
#Observations
This graph also illustrates the traffic in and out of Austin to several other destinations


```{r ans b-1-alpha-3, include=TRUE, echo=FALSE, fig.height=10, fig.width=15}
########################
#creating a graph
origin <- abia %>%
  distinct(Origin) %>%
...
per_route <- abia %>%  
  group_by(Origin, Dest) %>%
  summarise(mean_dep_delay = mean(DepDelay, na.rm=TRUE), mean_arr_delay = mean(ArrDelay, na.rm=TRUE)) %>% 
  ungroup()
per_route
...
edges<-edges%>%mutate(mean_delay = case_when(
  Origin == 'AUS' ~ mean_dep_delay,
  Dest == 'AUS' ~ mean_arr_delay,
))

edges<- select(edges, from, to, mean_delay)
...
ggraph(routes_tidy, layout = "graphopt") + 
  geom_node_point() +
  geom_edge_link(aes(width = mean_delay), alpha = 1) + 
  scale_edge_width(range = c(0.2, 2)) +
  geom_node_text(aes(label = label), repel = TRUE) +
  labs(edge_width = "Delay in minutes both arrival and departure") +
  theme_graph(foreground = 'steelblue', fg_text_colour = 'white', base_family = 'Helvetica')
```

#For flights coming into Austin, we are looking at the arrival delay
#For flights departing from Austin, we are looking at the departure delay
#Each line represents the delay time. 
...
library(EBImage)
img = readImage("Naive_Bayes_plot.png")
display(img, method = "raster")
